...
它不是现有 Web 页面加壳，也不是 `packages/ui` 里的 Capacitor Android 包。这个包有自己独立的 React Native 入口、Android 原生工程、状态管理和通知桥接，用来提供面向手机的管理员值班视图和普通用户任务视图。

## 当前能力

- 首次连接页，支持保存后端地址并选择管理员或普通用户身份
- 本地恢复会话，保存后端 URL、登录模式和访问令牌
- 通过共享 client contract 调用 PMEOW Web 后端 REST API
...
移动端不单独维护接口定义，而是直接依赖 `@monitor/app-common`。
...
- Node.js 20 或更高版本
- 已在仓库根目录安装依赖
- 可用的 Android SDK、adb、JDK 和本地模拟器或真机
- PMEOW Web 后端已启动，并且你知道管理员密码或人员令牌

仓库根目录已经定义了 mobile 相关 workspace 脚本，优先从根目录执行即可。
...
这个脚本会先处理 `@monitor/server-contracts` 和 `@monitor/app-common` 的前置构建，再启动 React Native Metro bundler。
...
它会调用 `react-native run-android`，编译 Android 工程并把应用安装到当前连接的模拟器或设备。

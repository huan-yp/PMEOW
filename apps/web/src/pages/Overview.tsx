      ...
      <section>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-2xl border border-cyan-400/12 bg-cyan-500/[0.04] p-4 backdrop-blur-sm">
            ...
            <p className="mt-3 text-2xl font-semibold text-slate-100">{queuedCount + runningCount}</p>
            <p className="mt-2 text-xs text-slate-400">排队 {queuedCount} · 运行 {runningCount}</p>
          </div>
          ...
          <div className="rounded-2xl border border-amber-400/16 bg-amber-400/[0.05] p-4 backdrop-blur-sm">
            ...
            <p className="mt-3 text-2xl font-semibold text-amber-100">{openSecurityEvents.length}</p>
            <p className="mt-2 text-xs text-slate-300/80">
              {openSecurityEvents.length === 0 ? '过去 7 天未发现未处理事件' : `${riskyNodeCount} 个节点存在风险`}
            </p>
          </div>

          {/* Latest sampling card */}
          <div className="rounded-2xl border border-white/10 bg-slate-950/30 p-4 backdrop-blur-sm md:col-span-2 xl:col-span-4">
            <p className="text-xs uppercase tracking-[0.22em] text-slate-500">最新采样</p>
            <p className="mt-3 text-2xl font-semibold text-slate-100">
              {latestMetricTimestamp > 0 ? formatTime(latestMetricTimestamp) : '--'}
            </p>
            <p className="mt-2 text-xs text-slate-400">
              {latestMetricTimestamp > 0 ? '来自节点实时指标' : '等待第一批数据上报'}
            </p>
          </div>
        </div>
      </section>

import { describe, expect, it } from 'vitest';
import { buildProcessAuditRows } from '../../src/security/audit.js';
import { analyzeSecuritySnapshot, buildSecurityFingerprint } from '../../src/security/analyzer.js';
import type { StoredGpuUsageRow } from '../../src/db/gpu-usage.js';
import type { MetricsSnapshot, ProcessAuditRow } from '../../src/types.js';

function createSnapshot(): MetricsSnapshot {
  return {
    serverId: 'server-1',
    timestamp: 1_712_010_000,
    cpu: {
      usagePercent: 40,
      coreCount: 16,
      modelName: 'Threadripper',
      frequencyMhz: 3600,
      perCoreUsage: [35, 45],
    },
    memory: {
      totalMB: 65_536,
      usedMB: 20_480,
      availableMB: 45_056,
      usagePercent: 31.25,
      swapTotalMB: 8_192,
      swapUsedMB: 0,
      swapPercent: 0,
    },
    disk: {
      disks: [],
      ioReadKBs: 0,
      ioWriteKBs: 0,
    },
    network: {
      rxBytesPerSec: 0,
      txBytesPerSec: 0,
      interfaces: [],
    },
    gpu: {
      available: true,
      totalMemoryMB: 24_576,
      usedMemoryMB: 4_096,
      memoryUsagePercent: 16.67,
      utilizationPercent: 30,
      temperatureC: 65,
      gpuCount: 1,
    },
    processes: [],
    docker: [],
    system: {
      hostname: 'gpu-01',
      uptime: '1 day',
      loadAvg1: 1,
      loadAvg5: 1.5,
      loadAvg15: 2,
      kernelVersion: '6.8.0',
    },
  };
}

function createGpuRow(overrides: Partial<StoredGpuUsageRow>): StoredGpuUsageRow {
  return {
    id: 1,
    serverId: 'server-1',
    timestamp: 1_712_010_000,
    gpuIndex: 0,
    ownerType: 'unknown',
    ownerId: undefined,
    userName: 'bob',
    taskId: undefined,
    pid: 202,
    command: 'xmrig --donate-level=1',
    usedMemoryMB: 1_024,
    declaredVramMB: undefined,
    ...overrides,
  };
}

describe('buildSecurityFingerprint', () => {
  it('is stable for the same semantic details', () => {
    const fingerprintA = buildSecurityFingerprint('server-1', 'suspicious_process', {
      pid: 202,
      reason: '命中关键词 xmrig',
      keyword: 'xmrig',
      command: 'xmrig --donate-level=1',
      user: 'bob',
      usedMemoryMB: 1024,
    });
    const fingerprintB = buildSecurityFingerprint('server-1', 'suspicious_process', {
      usedMemoryMB: 1024,
      user: 'bob',
      command: 'xmrig --donate-level=1',
      keyword: 'xmrig',
      reason: '命中关键词 xmrig',
      pid: 202,
    });

    expect(fingerprintA).toBe(fingerprintB);
  });
});

describe('analyzeSecuritySnapshot', () => {
  it('creates separate findings for keyword and unowned reasons on the same row', () => {
    const snapshot = createSnapshot();
    const auditRows: ProcessAuditRow[] = [
      {
        pid: 202,
        user: 'bob',
        command: 'xmrig --donate-level=1',
        cpuPercent: 88,
        memPercent: 2.4,
        rss: 2_048,
        gpuMemoryMB: 1_024,
        ownerType: 'unknown',
        taskId: undefined,
        suspiciousReasons: ['命中关键词 xmrig', '无主 GPU 占用 45 分钟'],
      },
    ];

    expect(analyzeSecuritySnapshot({ snapshot, auditRows })).toEqual([
      {
        serverId: 'server-1',
        eventType: 'suspicious_process',
        fingerprint: buildSecurityFingerprint('server-1', 'suspicious_process', {
          reason: '命中关键词 xmrig',
          pid: 202,
          user: 'bob',
          command: 'xmrig --donate-level=1',
          taskId: undefined,
          keyword: 'xmrig',
          usedMemoryMB: 1_024,
        }),
        details: {
          reason: '命中关键词 xmrig',
          pid: 202,
          user: 'bob',
          command: 'xmrig --donate-level=1',
          taskId: undefined,
          keyword: 'xmrig',
          usedMemoryMB: 1_024,
        },
      },
      {
        serverId: 'server-1',
        eventType: 'unowned_gpu',
        fingerprint: buildSecurityFingerprint('server-1', 'unowned_gpu', {
          reason: '无主 GPU 占用 45 分钟',
          pid: 202,
          user: 'bob',
          command: 'xmrig --donate-level=1',
          taskId: undefined,
          usedMemoryMB: 1_024,
        }),
        details: {
          reason: '无主 GPU 占用 45 分钟',
          pid: 202,
          user: 'bob',
          command: 'xmrig --donate-level=1',
          taskId: undefined,
          usedMemoryMB: 1_024,
        },
      },
    ]);
  });

  it('does not fall back to legacy suspiciousReasons when structured audit metadata exists but is empty', () => {
    const snapshot = createSnapshot();
    const auditRows = buildProcessAuditRows(
      {
        ...snapshot,
        processes: [
          {
            pid: 404,
            user: 'alice',
            command: 'python trainer.py',
            cpuPercent: 18,
            memPercent: 1.2,
            rss: 1_024,
          },
        ],
      },
      [],
      {
        securityMiningKeywords: ['xmrig'],
        unownedGpuMinutes: 45,
        hasRunningPmeowTasks: true,
        highGpuUtilizationActive: false,
      },
    );

    expect(auditRows).toEqual([
      {
        pid: 404,
        user: 'alice',
        command: 'python trainer.py',
        cpuPercent: 18,
        memPercent: 1.2,
        rss: 1_024,
        gpuMemoryMB: 0,
        ownerType: 'none',
        taskId: undefined,
        suspiciousReasons: [],
      },
    ]);

    auditRows[0]!.suspiciousReasons = ['命中关键词 xmrig'];

    expect(analyzeSecuritySnapshot({ snapshot, auditRows })).toEqual([]);
  });

  it('derives event type, keyword, details, and fingerprint from structured findings instead of labels', () => {
    const snapshot = createSnapshot();
    const auditRows = buildProcessAuditRows(
      {
        ...snapshot,
        processes: [
          {
            pid: 202,
            user: 'bob',
            command: 'xmrig --donate-level=1',
            cpuPercent: 88,
            memPercent: 2.4,
            rss: 2_048,
          },
        ],
      },
      [createGpuRow({})],
      {
        securityMiningKeywords: ['xmrig'],
        unownedGpuMinutes: 45,
        hasRunningPmeowTasks: false,
        highGpuUtilizationActive: false,
      },
    );

    auditRows[0]!.suspiciousReasons = ['关键词告警: xmrig', 'GPU 无归属占用 45 分钟'];

    expect(analyzeSecuritySnapshot({ snapshot, auditRows })).toEqual([
      {
        serverId: 'server-1',
        eventType: 'suspicious_process',
        fingerprint: buildSecurityFingerprint('server-1', 'suspicious_process', {
          reason: '命中关键词 xmrig',
          pid: 202,
          user: 'bob',
          command: 'xmrig --donate-level=1',
          taskId: undefined,
          keyword: 'xmrig',
          usedMemoryMB: 1_024,
        }),
        details: {
          reason: '命中关键词 xmrig',
          pid: 202,
          user: 'bob',
          command: 'xmrig --donate-level=1',
          taskId: undefined,
          keyword: 'xmrig',
          usedMemoryMB: 1_024,
        },
      },
      {
        serverId: 'server-1',
        eventType: 'unowned_gpu',
        fingerprint: buildSecurityFingerprint('server-1', 'unowned_gpu', {
          reason: '无主 GPU 占用 45 分钟',
          pid: 202,
          user: 'bob',
          command: 'xmrig --donate-level=1',
          taskId: undefined,
          usedMemoryMB: 1_024,
        }),
        details: {
          reason: '无主 GPU 占用 45 分钟',
          pid: 202,
          user: 'bob',
          command: 'xmrig --donate-level=1',
          taskId: undefined,
          usedMemoryMB: 1_024,
        },
      },
    ]);
  });
});
...
| `server:getTaskAuditDetail` | `taskId` | 拉取任务审计详情 |
| `server:pauseQueue` | 无或空 payload | 暂停队列 |
| `server:resumeQueue` | 无或空 payload | 恢复队列 |
| `server:setPriority` | `taskId`, `priority` | 调整优先级 |

**重要约束**：Web 到 Agent 的所有读写命令只允许通过 live session 上的 WebSocket command 传输。服务端不得对 Agent 暴露的任何 HTTP 地址发起请求来获取数据，也不得为任何命令新增 HTTP fallback。

### 命令错误映射

所有 server 到 agent 命令共享统一的错误分类和 HTTP 状态码映射：

| 错误类型 | HTTP 状态码 | 含义 |
| --- | --- | --- |
| `offline` | 409 | Agent 未在线，没有 live session |
| `timeout` | 504 | Agent 响应超时 |
| `not_supported` | 501 | Agent 版本不支持该命令 |
| `not_found` | 404 | 任务不存在 |
| `invalid_target` | 409 | 目标节点不是 agent 类型或不支持该命令 |
| `invalid_input` | 400 | 参数非法 |
| `internal` | 500 | 未分类的服务端缺陷 |

这些错误由 `AgentCommandService` 统一分类，route facade 仅负责将其映射为 HTTP 响应。不允许使用字符串匹配 `error.message` 来判断错误类型。

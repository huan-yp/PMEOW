...
| **SocketServer** | 本地 Unix socket IPC，供 CLI 提交任务、查询队列、attached_python 交互 |
| **RuntimeMonitor** | 1s 线程扫描活跃任务进程树，检测 orphan / 异常退出 |
| **TaskRunner** | 管理子进程的启动和收割（daemon_shell 模式） |
...
| 命令 | 说明 |
|------|------|
| `submit` | 提交任务（daemon_shell 或 attached_python） |
| `list` | 查询当前任务队列 |
| `cancel` | 取消任务 |
| `confirm_attached_launch` | attached_python: CLI 回报已启动的 PID |
| `finish_attached_task` | attached_python: CLI 回报任务完成 |
| `query_task_status` | attached_python: CLI 轮询任务状态 |
...
class TaskRecord:
    ...
    cwd: str
    user: str
    launch_mode: TaskLaunchMode     # daemon_shell | attached_python
...
| **attached_python 超时无 PID** | `reserved` 状态且 attach_deadline 过期 → 任务从活跃队列移除 |
...
| **集成测试** | attached_python 超时 | 模拟 CLI 不回报 PID，验证超时处理 |
...
├── models.py                    # [重写] 数据模型定义
├── config.py                    # [改] 更新配置项
├── cli_python.py                # [改] attached_python CLI，适配无 launching 流程
...
- `TaskLaunchMode`: `daemon_shell | attached_python`
